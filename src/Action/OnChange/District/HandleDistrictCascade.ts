...
export function HandleDistrictCascade(context: Xrm.Events.EventContext) {
    ...
    const districtAttr =
        formContext.getAttribute<Xrm.Attributes.LookupAttribute>('sua_district');
    ...
    console.debug(
        `HandleDistrictCascade: district is ${district.name} (${district.id})`
    );
    ...
    entities.forEach((entity) => {
        ...
        if (!attribute) {
            console.error(
                `HandleDistrictCascade: attribute ${entity.attributeName} not found on form`
            );
            return;
        }
        const viewId = uuidv4();
        ...
        console.debug(
            `HandleDistrictCascade: added custom view for ${entity.attributeName} (viewId=${viewId})`
        );
    });
}
